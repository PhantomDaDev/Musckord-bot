...
module.exports = {
  ...
  run: async ({ interaction }) => {
    try {
      ...
      const reply = await interaction.reply({
        content: `${targetUser}, you have been challeneged to play a game of Rock Paper Scissors by ${interaction.user}. To start playing, click one of the buttons below.`,
        embeds: [embed],
        components: [row],
      });
      ...
      if (targetUser.beats === initialUserChoice.name) {
        result = `${targetUser} won!`;
      }
      
      if (initialUserChoice.beats === targetUser.name) {
        result = `${interaction.user} won!`;
      }
      
      if (targetUser.name === initialUserChoice.name) {
        result = "It was a tie!";
      }
      
      embed.setDescription(`
        ${targetUser.toString()} picked ${targetUserChoice.name + targetUserChoice.emoji}
        ${interaction.user.toString()} picked ${initialUserChoice.name + initialUserChoice.emoji}
        \n\n${result}
      `);
      ...
      console.warn(`Error handling /rps: ${error}`);
    }
  },
  ...
  data: {
    ...
    options: [
      {
        name: "user",
        description: "The person you want to play with",
